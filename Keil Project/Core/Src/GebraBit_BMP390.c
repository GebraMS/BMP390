...
{	
	uint8_t txBuf[3] = {regAddr|0x80 , 0xFF, 0xFF}; //Read operation: set the 8th-bit to 1.
	uint8_t rxBuf[3];
...
uint8_t GB_BMP390_Burst_Read(uint8_t regAddr,  uint8_t *data, uint16_t byteQuantity)
{
	...
	uint8_t *pRxBuf;
	uint8_t status = HAL_ERROR;
	pTxBuf = ( uint8_t * )malloc(sizeof(uint8_t) * (byteQuantity + 3)); // reason of "+1" is for register address that comes in first byte
	pRxBuf = ( uint8_t * )malloc(sizeof(uint8_t) * (byteQuantity + 3));
	memset(pTxBuf, 0, (byteQuantity + 3)*sizeof(uint8_t));

	pTxBuf[0] = regAddr | 0x80; //Read operation: set the 8th-bit to 1.
	pTxBuf[1] = 0xFF;//Dummy
	pTxBuf[2] = 0xFF;//Dummy
	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_RESET);
	status = (HAL_SPI_TransmitReceive(&hspi1, pTxBuf, pRxBuf, byteQuantity+3, HAL_MAX_DELAY));
	HAL_GPIO_WritePin(SPI_CS_GPIO_Port, SPI_CS_Pin, GPIO_PIN_SET);
	
	if (status == HAL_OK)
	{
		memcpy(data, &pRxBuf[2], byteQuantity*sizeof(uint8_t)); //here we dont have "+1" beacause we don't need first byte that was register data , we just need DATA itself
	}
	free(pTxBuf);
	free(pRxBuf);
	return status;
...
{
	GB_BMP390_Read_Reg_Data( BMP390_STATUS,&BMP390->SENSOR_STATUS);
}
/*=========================================================================================================================================
...
{
	GB_BMP390_Read_Reg_Data( BMP390_ERR_REG,&BMP390->ERROR_CONDITION);
}
/*=========================================================================================================================================
